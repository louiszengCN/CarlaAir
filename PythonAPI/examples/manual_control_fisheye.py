...
class World:
    def __init__(self, carla_world, hud, args) -> None:
        ...
        self._equirectangular = args.equirectangular
        self._perspective = args.perspective
    ...
    def next_weather(self, reverse=False) -> None:
    ...
    def next_map_layer(self, reverse=False) -> None:
        self.current_map_layer += -1 if reverse else 1
        self.current_map_layer %= len(self.map_layer_names)
        selected = self.map_layer_names[self.current_map_layer]
        self.hud.notification(f"LayerMap selected: {selected}")

    def load_map_layer(self, unload=False) -> None:
        selected = self.map_layer_names[self.current_map_layer]
        if unload:
    ...
    def modify_vehicle_physics(self, actor) -> None:
...
class KeyboardControl:
    ...
    def parse_events(self, client, world, clock, sync_mode) -> Optional[bool]:
    ...
    def _parse_walker_keys(self, keys, milliseconds, world) -> None:
...
class HUD:
    ...
    def tick(self, world, clock) -> None:
    ...
    def show_ackermann_info(self, enabled) -> None:
        self._show_ackermann_info = enabled

    def update_ackermann_control(self, ackermann_control) -> None:
...
class GnssSensor:
    ...
    @staticmethod
    def _on_gnss_event(weak_self, event) -> None:
        self = weak_self()
        if not self:
            return
...
class IMUSensor:
    ...
    @staticmethod
    def _IMU_callback(weak_self, sensor_data) -> None:
        self = weak_self()
        if not self:
            return
        limits = (-99.9, 99.9)
...
class RadarSensor:
    ...
    @staticmethod
    def _Radar_callback(weak_self, radar_data) -> None:
        self = weak_self()
        if not self:
            return
...
class CameraManager:
    def __init__(self, parent_actor, hud, gamma_correction,
                 ...
                 longitude_shift, kannala_brandt_params) -> None:
        self.sensor = None
        self.surface = None
        self._parent = parent_actor
        self.hud = hud
        ...
        self.sensors = [
            ["sensor.camera.rgb.wide_angle_lens", cc.Raw, "Wide Angle Lens Camera", {}],
            ["sensor.camera.depth.wide_angle_lens", cc.Raw, "Camera Depth (Raw, Wide Angle Lens)", {}],
            ["sensor.camera.depth.wide_angle_lens", cc.Depth, "Camera Depth (Gray Scale, Wide Angle Lens)", {}],
            ["sensor.camera.depth.wide_angle_lens", cc.LogarithmicDepth, "Camera Depth (Logarithmic Gray Scale, Wide Angle Lens)", {}],
            ["sensor.camera.semantic_segmentation.wide_angle_lens", cc.Raw, "Camera Semantic Segmentation (Raw, Wide Angle Lens)", {}],
        ...
        for item in self.sensors:
            ...
            if item[0].startswith("sensor.camera"):
                ...
                if item[0].endswith("wide_angle_lens"):
                    if bp.has_attribute("fov"):
                        bp.set_attribute("fov", str(fov))
                    if bp.has_attribute("camera_model"):
                        bp.set_attribute("camera_model", str(model))
                    if bp.has_attribute("equirectangular"):
                        bp.set_attribute("equirectangular", str(equirectangular))
                    if bp.has_attribute("perspective"):
                        bp.set_attribute("perspective", str(perspective))
                    if bp.has_attribute("fov_mask"):
                        bp.set_attribute("fov_mask", str(fov_mask))
                    if bp.has_attribute("fov_fade_size"):
    ...
    def set_sensor(self, index, notify=True, force_respawn=False) -> None:
...
def main() -> None:
    ...
    argparser.add_argument(
        "--equirectangular",
        action="store_true",
        help='Determines whether image rectification will be applied (default: "false")')
    argparser.add_argument(
        "--perspective",
        action="store_true",
        help='Determines whether image rectification will be applied (default: "false")')
