...
void MavLinkVehicleImpl::handleMessage(std::shared_ptr<MavLinkConnection> connection, const MavLinkMessage& msg)
{
    ...
    case MavLinkHeartbeat::kMessageId: { // MAVLINK_MSG_ID_HEARTBEAT:
        ...
        bool armed = (heartbeat.base_mode & static_cast<uint8_t>(mavlinkcom::MAV_MODE_FLAG::MAV_MODE_FLAG_SAFETY_ARMED)) != 0;
        std::lock_guard<std::mutex> guard(state_mutex_);
        if (vehicle_state_.controls.armed != armed) {
            state_version_++;
            vehicle_state_.controls.armed = armed;
        }
        if (heartbeat.autopilot == static_cast<uint8_t>(mavlinkcom::MAV_AUTOPILOT::MAV_AUTOPILOT_PX4)) {
    ...
    case MavLinkExtendedSysState::kMessageId: { // MAVLINK_MSG_ID_EXTENDED_SYS_STATE:
        ...
        bool landed = extstatus.landed_state == static_cast<int>(mavlinkcom::MAV_LANDED_STATE::MAV_LANDED_STATE_ON_GROUND);
        std::lock_guard<std::mutex> guard(state_mutex_);
    ...
    case MavLinkCommandAck::kMessageId: {
        ...
        if (ack.command == MavCmdNavGuidedEnable::kCommandId) {
            mavlinkcom::MAV_RESULT ackResult = static_cast<mavlinkcom::MAV_RESULT>(ack.result);
            if (ackResult == mavlinkcom::MAV_RESULT::MAV_RESULT_TEMPORARILY_REJECTED) {
                Utils::log("### command MavCmdNavGuidedEnable result: MAV_RESULT_TEMPORARILY_REJECTED");
            }
            else if (ackResult == mavlinkcom::MAV_RESULT::MAV_RESULT_UNSUPPORTED) {
                Utils::log("### command MavCmdNavGuidedEnable result: MAV_RESULT_UNSUPPORTED");
                vehicle_state_.controls.offboard = false;
            }
            else if (ackResult == mavlinkcom::MAV_RESULT::MAV_RESULT_FAILED) {
                Utils::log("### command MavCmdNavGuidedEnable result: MAV_RESULT_FAILED");
                vehicle_state_.controls.offboard = false;
            }
            else if (ackResult == mavlinkcom::MAV_RESULT::MAV_RESULT_ACCEPTED) {
...
void MavLinkVehicleImpl::releaseControl()
{
    ...
    control_request_sent_ = false;
    vehicle_state_.controls.offboard = false;
    setMode(static_cast<int>(mavlinkcom::MAV_MODE_FLAG::MAV_MODE_FLAG_CUSTOM_MODE_ENABLED),
...
void MavLinkVehicleImpl::offboardIdle()
{
    ...
    msg.coordinate_frame = static_cast<uint8_t>(mavlinkcom::MAV_FRAME::MAV_FRAME_LOCAL_NED);
...
void MavLinkVehicleImpl::moveToGlobalPosition(float lat, float lon, float alt, bool isYaw, float yawOrRate)
{
    ...
    msg.coordinate_frame = static_cast<uint8_t>(mavlinkcom::MAV_FRAME::MAV_FRAME_GLOBAL_INT);
...
AsyncResult<bool> MavLinkVehicleImpl::setMode(int mode, int customMode, int customSubMode, bool waitForAck)
{
    ...
    if ((vehicle_state_.mode & static_cast<int>(mavlinkcom::MAV_MODE_FLAG::MAV_MODE_FLAG_HIL_ENABLED)) != 0) {
        mode |= static_cast<int>(mavlinkcom::MAV_MODE_FLAG::MAV_MODE_FLAG_HIL_ENABLED); // must preserve this flag.
    }
    if ((vehicle_state_.mode & static_cast<uint8_t>(mavlinkcom::MAV_MODE_FLAG::MAV_MODE_FLAG_SAFETY_ARMED)) != 0) {
        mode |= static_cast<int>(mavlinkcom::MAV_MODE_FLAG::MAV_MODE_FLAG_SAFETY_ARMED); // must preserve this flag.
    }
...
{
    return setMode(static_cast<int>(mavlinkcom::MAV_MODE_FLAG::MAV_MODE_FLAG_CUSTOM_MODE_ENABLED),
                   static_cast<int>(PX4_CUSTOM_MAIN_MODE_POSCTL));
}

AsyncResult<bool> MavLinkVehicleImpl::setStabilizedFlightMode()
{
    return setMode(static_cast<int>(mavlinkcom::MAV_MODE_FLAG::MAV_MODE_FLAG_STABILIZE_ENABLED));
}
...
{
    return setMode(static_cast<int>(mavlinkcom::MAV_MODE_FLAG::MAV_MODE_FLAG_CUSTOM_MODE_ENABLED) |
                       static_cast<int>(mavlinkcom::MAV_MODE_FLAG::MAV_MODE_FLAG_AUTO_ENABLED),
...
bool MavLinkVehicleImpl::isLocalControlSupported()
{
    ...
    return (cap.capabilities & static_cast<int>(mavlinkcom::MAV_PROTOCOL_CAPABILITY::MAV_PROTOCOL_CAPABILITY_SET_POSITION_TARGET_LOCAL_NED)) != 0;
}
...
bool MavLinkVehicleImpl::isAttitudeControlSupported()
{
    ...
    return (cap.capabilities & static_cast<int>(mavlinkcom::MAV_PROTOCOL_CAPABILITY::MAV_PROTOCOL_CAPABILITY_SET_ATTITUDE_TARGET)) != 0;
}
