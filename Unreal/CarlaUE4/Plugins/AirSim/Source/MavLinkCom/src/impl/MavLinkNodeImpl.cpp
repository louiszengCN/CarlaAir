...
void MavLinkNodeImpl::sendOneHeartbeat()
{
    ...
    heartbeat.autopilot = static_cast<uint8_t>(mavlinkcom::MAV_AUTOPILOT::MAV_AUTOPILOT_GENERIC);
...
float UnpackParameter(uint8_t type, float param_value)
{
    ...
    float value = 0;
    switch (static_cast<mavlinkcom::MAV_PARAM_TYPE>(type)) {
    case mavlinkcom::MAV_PARAM_TYPE::MAV_PARAM_TYPE_UINT8:
        value = static_cast<float>(pu.ub);
        break;
    case mavlinkcom::MAV_PARAM_TYPE::MAV_PARAM_TYPE_INT8:
        value = static_cast<float>(pu.b);
        break;
    case mavlinkcom::MAV_PARAM_TYPE::MAV_PARAM_TYPE_UINT16:
        value = static_cast<float>(pu.us);
        break;
    case mavlinkcom::MAV_PARAM_TYPE::MAV_PARAM_TYPE_INT16:
        value = static_cast<float>(pu.s);
        break;
    case mavlinkcom::MAV_PARAM_TYPE::MAV_PARAM_TYPE_UINT32:
        value = static_cast<float>(pu.ui);
        break;
    case mavlinkcom::MAV_PARAM_TYPE::MAV_PARAM_TYPE_INT32:
        value = static_cast<float>(pu.i);
        break;
    case mavlinkcom::MAV_PARAM_TYPE::MAV_PARAM_TYPE_UINT64:
        // we only have 4 bytes for the value in mavlink_param_value_t, so how does this one work?
        value = static_cast<float>(pu.ui);
        break;
    case mavlinkcom::MAV_PARAM_TYPE::MAV_PARAM_TYPE_INT64:
        // we only have 4 bytes for the value in mavlink_param_value_t, so how does this one work?
        value = static_cast<float>(pu.i);
        break;
    case mavlinkcom::MAV_PARAM_TYPE::MAV_PARAM_TYPE_REAL32:
        value = param_value;
        break;
    case mavlinkcom::MAV_PARAM_TYPE::MAV_PARAM_TYPE_REAL64:
        // we only have 4 bytes for the value in mavlink_param_value_t, so how does this one work?
        value = param_value;
        break;
    default:
    ...
    int subscription = con->subscribe([=](std::shared_ptr<MavLinkConnection> connection, const MavLinkMessage& message) {
        ...
        if (message.msgid == MavLinkCommandAck::kMessageId) {
            ...
            if (ack.command == cmd) {
                ...
                result.setResult(ackResult == mavlinkcom::MAV_RESULT::MAV_RESULT_ACCEPTED);
            }
        }
    });
    result.setState(subscription);
