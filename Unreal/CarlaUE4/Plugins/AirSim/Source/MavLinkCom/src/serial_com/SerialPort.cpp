...
public:
    ...
    int connect(const char* portName, int baudRate)
    {
        ...
        handle = CreateFileW(wide.c_str(), GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, 0);
...
int SerialPort::write(const uint8_t* ptr, int count)
{
    return impl_->write(ptr, count);
}

int SerialPort::read(uint8_t* buffer, int bytesToRead)
{
