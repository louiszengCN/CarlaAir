...
void UCarlaSettingsDelegate::OnActorSpawned(AActor *InActor)
{
  ...
  if (InActor != nullptr && IsValid(InActor) && IsValid(InActor) &&
...
void UCarlaSettingsDelegate::ApplyQualityLevelPreRestart()
{
  CheckCarlaSettings(nullptr);
  UWorld *InWorld = CarlaSettings->GetWorld();
  if (!IsValid(InWorld) || !IsValid(InWorld))
  {
    return;
  }
...
void UCarlaSettingsDelegate::CheckCarlaSettings(UWorld *world)
{
  ...
  if (world == nullptr || !IsValid(world) || !IsValid(world))
  {
    world = GetLocalWorld();
  }
  check(world != nullptr);
...
{
  if (!world || !IsValid(world) || !IsValid(world))
  {
    return;
  }
  AsyncTask(ENamedThreads::GameThread, [=, this]() { // UE5: explicit this capture required
    if (!world || !IsValid(world) || !IsValid(world))
    {
      return;
    }
    ...
    for (int32 i = 0; i < actors.Num(); i++)
    {
      ...
      if (!IsValid(actor) || !IsValid(actor))
      {
        continue;
      }
    ...
    const bool hide_non_directional) const
{
  ...
  AsyncTask(ENamedThreads::GameThread, [=, this]() { // UE5: explicit this capture required
    ...
    for (int32 i = 0; i < actors.Num(); i++)
    {
      if (!IsValid(actors[i]) || !IsValid(actors[i]))
      {
        continue;
      }
