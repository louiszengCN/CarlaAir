...
void UCarlaSettingsDelegate::ApplyQualityLevelPreRestart()
{
  CheckCarlaSettings(nullptr);
  UWorld *InWorld = CarlaSettings->GetWorld();
  if (!IsValid(InWorld) || IsGarbage(InWorld))
  {
    return;
  }
...
void UCarlaSettingsDelegate::CheckCarlaSettings(UWorld *world)
{
  ...
  if (world == nullptr || !IsValid(world) || IsGarbage(world))
  {
    world = GetLocalWorld();
  }
  check(world != nullptr);
...
{
  if (!world || !IsValid(world) || IsGarbage(world))
  {
    return;
  }
  AsyncTask(ENamedThreads::GameThread, [=]() {
    ...
    for (int32 i = 0; i < actors.Num(); i++)
    {
      ...
      if (!IsValid(actor) || IsGarbage(actor))
      {
        continue;
      }
    ...
    const bool hide_non_directional) const
{
  ...
  AsyncTask(ENamedThreads::GameThread, [=]() {
    ...
    for (int32 i = 0; i < actors.Num(); i++)
    {
      if (!IsValid(actors[i]) || IsGarbage(actors[i]))
      {
        continue;
      }
