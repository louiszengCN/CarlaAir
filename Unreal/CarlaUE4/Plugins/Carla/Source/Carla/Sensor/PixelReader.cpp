...
// Throttle concurrent GPU readbacks to prevent resource exhaustion.
// Without this limit, 4+ camera sensors accumulate pending readback
// operations over time, eventually crashing the server after ~4 minutes.
// The limit allows a healthy pipeline depth while preventing unbounded growth.
static std::atomic<int32> GPendingReadbacks(0);
static constexpr int32 GMaxPendingReadbacks = 16;
    ...
    FPixelReader::Payload FuncForSending)
{
  ...
  // that causes server crash with 4+ concurrent sensors.
  if (GPendingReadbacks.load(std::memory_order_relaxed) >= GMaxPendingReadbacks)
  {
    return;
  }
  ...
  AsyncTask(ENamedThreads::HighTaskPriority, [=, Readback=std::move(BackBufferReadback)]() mutable {
    ...
    GPendingReadbacks.fetch_sub(1, std::memory_order_relaxed);
  });
}
