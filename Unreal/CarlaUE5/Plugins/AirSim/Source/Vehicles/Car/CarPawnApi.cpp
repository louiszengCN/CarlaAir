...
{
    movement_ = Cast<UChaosWheeledVehicleMovementComponent>(pawn->GetVehicleMovement());
}
...
void CarPawnApi::reset()
{
    ...
    UAirBlueprintLib::RunCommandOnGameThread([this, &phys_comps]() {
        ...
        movement_->SetThrottleInput(0.0f);
        movement_->SetSteeringInput(0.0f);
        movement_->SetBrakeInput(0.0f);
        movement_->SetHandbrakeInput(false);
        movement_->SetUseAutomaticGears(true);
