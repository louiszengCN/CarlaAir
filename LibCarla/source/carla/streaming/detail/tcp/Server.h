        ...
        public:
          ...
          void Listen(FunctorT1 on_session_opened, FunctorT2 on_session_closed) {
            boost::asio::post(_io_context, [=, this]() { // C++20: explicit this capture required
              OpenSession(_timeout, std::move(on_session_opened), std::move(on_session_closed));
            });
          }

          void SetSynchronousMode(bool is_synchro) { _synchronous = is_synchro; }

          bool IsSynchronousMode() const { return _synchronous; }

        private:
          void OpenSession(time_duration timeout, ServerSession::callback_function_type on_session_opened,
