...
namespace carla {
  namespace multigpu {
    ...
    PrimaryCommands::PrimaryCommands(std::shared_ptr<Router> router) : _router(router) {}
    ...
    token_type PrimaryCommands::SendGetToken(stream_id stream_id) {
      ...
      carla::Buffer buf((carla::Buffer::value_type *)&stream_id, (uint64_t)sizeof(stream_id));
    ...
    void PrimaryCommands::SendEnableForROS(carla::streaming::detail::stream_actor_id_type stream_actor_id) {
      ...
      if (it != _servers.end()) {
        carla::Buffer buf((carla::Buffer::value_type *)&stream_actor_id, (uint64_t)(sizeof(stream_actor_id)));
    ...
    token_type PrimaryCommands::GetToken(stream_id stream_id) {
      ...
      if (it != _tokens.end()) {
        ...
        log_debug("Using token from already activated sensor: ", it->second.get_stream_id(), ", ",
                  it->second.get_port());
        return it->second;
      } else {
