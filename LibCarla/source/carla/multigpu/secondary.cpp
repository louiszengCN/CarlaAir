...
namespace carla {
  namespace multigpu {
    ...
    Secondary::Secondary(boost::asio::ip::tcp::endpoint ep, SecondaryCommands::callback_type callback)
        : _pool(), _socket(_pool.io_context()), _endpoint(ep), _strand(_pool.io_context()),
          _connection_timer(_pool.io_context()), _buffer_pool(std::make_shared<BufferPool>()) {

      _commander.set_callback(callback);
    }

    Secondary::Secondary(std::string ip, uint16_t port, SecondaryCommands::callback_type callback)
        : _pool(), _socket(_pool.io_context()), _strand(_pool.io_context()), _connection_timer(_pool.io_context()),
          _buffer_pool(std::make_shared<BufferPool>()) {

      boost::asio::ip::address ip_address = boost::asio::ip::make_address(ip); // Boost 1.66+: from_string removed
    ...
    void Secondary::Write(std::string text) {
      ...
      boost::asio::post(_strand, [=]() {
        ...
        boost::asio::async_write(self->_socket, boost::asio::buffer(&this_size, sizeof(this_size)),
                                 boost::asio::bind_executor(self->_strand, handle_sent));

        // send characters
        boost::asio::async_write(self->_socket, boost::asio::buffer(text.c_str(), text.size()),
                                 boost::asio::bind_executor(self->_strand, handle_sent));
      });
    }
    ...
    void Secondary::ReadData() {
      ...
      boost::asio::post(_strand, [weak]() {
        ...
        auto handle_read_header = [weak, message, handle_read_data](boost::system::error_code ec,
          ...
          if (!ec && (message->size() > 0u)) {
            ...
            boost::asio::async_read(self->_socket, message->buffer(),
        ...
        boost::asio::async_read(self->_socket, message->size_as_buffer(),
