...
namespace carla {
  namespace multigpu {
    ...
    public:
      ...
      endpoint GetLocalEndpoint() const { return _acceptor.local_endpoint(); }
      ...
      void Listen(callback_function_type on_session_opened, callback_function_type on_session_closed,
                  callback_function_type_response on_response) {
        boost::asio::post(_io_context, [=, this]() { // C++20: explicit this capture required
          OpenSession(_timeout, std::move(on_session_opened), std::move(on_session_closed), std::move(on_response));
        });
      }
    ...
    private:
      void OpenSession(time_duration timeout, callback_function_type on_session_opened,
                       callback_function_type on_session_closed, callback_function_type_response on_response);
