...
### New Features
 * Auto traffic generation: 30 vehicles + 50 pedestrians spawned on simulator startup
 * P key toggles between physics (realistic collision) and invincible (pass-through) mode
 * Ground clamping prevents drone from clipping through terrain

### Improvements
 * Replaced custom bounce collision system with UE4 native Sweep collision — drone now stops on impact instead of bouncing away
 * H overlay menu: fixed numpad +/- keybinding, compact layout, 18pt status bar font
 * CarlaAir.sh launcher: auto-detect Vulkan ICD, auto-kill previous instance, traffic auto-spawn

### Architecture
 * Unified ASimWorldGameMode merging CARLA (ACarlaGameModeBase) and AirSim (ASimModeBase) in a single GameMode
 * Dual API servers: CARLA RPC on port 2000, AirSim RPC on port 41451
